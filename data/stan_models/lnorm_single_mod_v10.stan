...
parameters {
  real<lower=0.1, upper = 4> mu_0;                      // lognormal mean parameter
  ...
  // real<lower=-3, upper = -0.5> mu_sigma;         // lognormal standard deviation parameter
...
model {
  ...
  for (s in 1:S){
    mu_species[s] ~ lognormal(0, 0.5);
    // sigma_species[s] ~ normal(0.0, 1);
  }
  ...
  for (n in 1:N) {
    ...
    log_size_mean = mu_0 + mu_loc*loc[n] + mu_year*year[n] + mu_species[species_indx[n]];
    sigma = exp(beta_0 + beta_1*log_size_mean);// + beta_0 + beta_1*log_size_mean);// + sigma_loc*loc[n] + sigma_year*year[n] + sigma_species[species_indx[n]]);
    // real mu = exp(log_size_mean);
    if (bin[n] == 1) { // probability in smallest bin (i.e. not recorded by RLS) 0-1.25cm
    ...
    // i think this is how you would rescale your probability
    // prob2 = prob/(lognormal_cdf(uprsize[1], log_size_mean, sigma) + prob);
