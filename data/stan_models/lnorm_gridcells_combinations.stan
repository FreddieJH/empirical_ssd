...
data {
  ...
  int<lower=1> n_combinations;   // number of combinations of years and latitudes
  ...
  int<lower=0, upper=n_combinations> combination_id[n_observations]; // ID of the year-by-location combination
  ...
  // real lat_id[n_observations]; // latitude ID
  // real yr_id[n_observations];  // year ID

}
 
parameters {
  vector<lower= -1.0, upper=1.0>[n_combinations]  ln_sigma; // coef of variation (needs to be exponentited first)
  vector<lower= 0.0, upper=4.0>[n_combinations] ln_mu; 
}
...
model {
  ...
  // ln_sigma_year ~ normal(0.0, 0.5); // log-normal prior on sigma
	...
	for(i in 1:n_observations){
	   sigma = exp(ln_sigma[combination_id[i]]);
	    
	    
	 // For the first size bin only (between 1.25 and 3.75)
    if (sizebin_id[i] == 1) { 
      
       // probability in smallest bin
       prob = lognormal_cdf(upper_sizebin[1], ln_mu[combination_id[i]], sigma); 
    ...
    // all other bins (>2.5cm)
      ...
      // equalling the probability of being within a specific bin.
       prob = lognormal_cdf(upper_sizebin[sizebin_id[i]], ln_mu[combination_id[i]], sigma) - 
         lognormal_cdf(upper_sizebin[sizebin_id[i]-1], ln_mu[combination_id[i]], sigma); 
    }
