...
model {
  ...
  for (n in 1:N) {
    ...
    sigma = exp(mu_sigma + sigma_species[species_indx[n]]);// + beta_0 + beta_1*log_size_mean);// + sigma_loc*loc[n] + sigma_year*year[n] + sigma_species[species_indx[n]]);
