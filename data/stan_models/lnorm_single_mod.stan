...
parameters {
  ...
  vector<lower=-2, upper = 2>[L] mu_loc;               // location effects on mean
  vector<lower=-2, upper = 2>[Y] mu_year;              // year effects on mean
  vector<lower=-2, upper = 2>[S] mu_species;           // species effects on mean
  real<lower=-1, upper = 1> mu_sigma;         // lognormal standard deviation parameter
  vector<lower=-1, upper = 1>[L] sigma_loc;   // location effects on standard deviation
  vector<lower=-1, upper = 1>[Y] sigma_year;  // year effects on standard deviation
  vector<lower=-1, upper = 1>[S] sigma_species; // species effects on standard deviation
}
...
model {
  ...
  for (l in 1:L)
  mu_loc[l] ~ normal(0.0, 1.0);
  for (y in 1:Y)
  mu_year[y] ~ normal(0.0, 1.0);
  for (s in 1:S)
  mu_species[s] ~ normal(0.0, 1.0);
  
  // Linear model for standard deviation
  for (l in 1:L)
  sigma_loc[l] ~ normal(0.0, 0.5);
  for (y in 1:Y)
  sigma_year[y] ~ normal(0.0, 0.5);
  for (s in 1:S)
  sigma_species[s] ~ normal(0.0, 0.5);
  
  // Likelihood
  for (n in 1:N) {
    sigma = exp(mu_sigma) + sigma_loc[loc_indx[n]]*loc[n] + sigma_year[year_indx[n]]*year[n];
    log_size_mean = mu_0 +  sigma_loc[loc_indx[n]]*loc[n] + sigma_year[year_indx[n]]*year[n];
    // real log_size_mean = mu_0 + mu_loc[loc_indx[n]]*loc[n] + mu_year[year_indx[n]]*year[n] + mu_species[species_indx[n]];
    // real log_sigma_mean = mu_sigma + sigma_loc[loc_indx[n]]*loc[n] + sigma_year[year_indx[n]]*year[n] + sigma_species[species_indx[n]];
